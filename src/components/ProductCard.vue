...
<template>
    <li class="p-4 list-none rounded-lg border border-gray-300 bg-white relative group hover:border-primary transition cursor-pointer">
        <a @click.prevent="toProduct(product.id)">
            ...
            <div class="absolute top-4 left-4 opacity-0 bg-white/70 backdrop-blur-md p-1 rounded-full border border-gray-400  -translate-y-2 group-hover:opacity-100 group-hover:-translate-y-0 transition-all">
            ...
            <div
                ...
                <h3 class="truncate text-xs px-2">{{ product.title }}</h3>
                <span class="text-white text-sm text-nowrap bg-primary rounded-full px-2 py-1">{{ product.price }} $</span>
            </div>
